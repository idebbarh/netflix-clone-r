...
function Profiles() {
  ...
  const profilesListElem = user?.userProfiles?.map((profile, index) => {
    return (
      <ProfileLink
        Icon={profile.profileIconUrl}
        title={profile.profileTitle}
        key={index}
        setActiveProfileHandler={() => setActiveProfileHandler(profile)}
        isManageProfilesMode={isManageProfilesMode}
        setIsEditProfilePageOpen={setIsEditProfilePageOpen}
        ...
        <>
          <div className="profiles__listProfiles">
            ...
            <div className="profiles__chooseProfile">
              {profilesListElem}
              {user?.userProfiles?.length < 4 && (
                <ProfileLink
                  isAddProfileBtn={true}
                  Icon={AddCircleIcon}
                  title="Add Profile"
                  addProfile={() => setIsAddProfileOpen(true)}
                />
              )}
            </div>
          </div>
          <button
            className={`profiles__manageProfilesBtn${
              isManageProfilesMode ? " profiles__manageProfilesBtn--done" : ""
            }`}
            onClick={() => setIsManageProfilesMode((prevState) => !prevState)}
          >
            {isManageProfilesMode ? "done" : "Manage Profiles"}
          </button>
        </>
      )}
    </div>
