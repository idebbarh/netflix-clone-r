...
function CardOptions({
  ...
  Icon,
  showType = null,
  isThisInFavList = null,
  isFromWatchList = null,
}) {
  ...
  const addToMyListHandler = async () => {
    ...
    await updateDoc(docRef, {
      userProfiles: user.userProfiles.map((profile) =>
         profile.profileTitle === user.userActiveProfile.profileTitle
          ? {
              ...profile,
              profileFavList: [
                ...profile.profileFavList,
                { id: data.id, showType: showType },
              ],
            }
          : profile
      ...
      userActiveProfile: {
        ...
        profileFavList: [
          ...user.userActiveProfile.profileFavList,
          { id: data.id, showType: showType },
        ],
      },
    });
    ...
    await updateDoc(docRef, {
      userProfiles: user.userProfiles.map((profile) =>
        profile.profileTitle === user.userActiveProfile.profileTitle
          ? {
              ...profile,
              profileFavList: profile.profileFavList.filter(
                (p) => p.id !== data.id
              ),
            }
          : profile
      ...
      userActiveProfile: {
        ...
        profileFavList: 
          user
          .userActiveProfile.profileFavList.filter((p) => p.id !== data.id),
      },
    });
  };
  return (
      ...
      style={
        isPlay
          ...
          : {
              ...
              marginLeft: `${isMoreInfo ? "auto" : 0}`,
            }
      }
      onMouseEnter={isRating ? () => setIsRatingOptionsOpen(true) : undefined}
      onMouseLeave={isRating ? () => setIsRatingOptionsOpen(false) : undefined}
      onClick={onClickHandler}
    >
      <Icon />
      <AnimatePresence mode="wait">
        {isRatingOptionsOpen && <RatingOptions key="ratingOptions" />}
      </AnimatePresence>
    </div>
  );
}
